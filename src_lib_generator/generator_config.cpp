...
const string GeneratorConfig::InputFont::DefaultFontName = "arial" ;
const string GeneratorConfig::InputFont::DefaultFontPath = "/usr/share/fonts/Arial.ttf" ;
...
const float  GeneratorConfig::DefaultRatioSpreadToGlyph     =  0.1f ;
...
static vector< pair< uint32_t, uint32_t > > processJSON_ranges( const nlohmann::json& data_array )
{
...
void GeneratorConfig::processJSON_input_fonts( const nlohmann::json& data_array )
{
    for ( const auto& data : data_array ) {
        ...
        InputFont inputFont;
        inputFont.processJSON( data );

        mInputFonts.push_back( inputFont );
    }
}

void GeneratorConfig::InputFont::processJSON( const nlohmann::json& data )
{
    if ( data.contains( JSON_KEY_FONT_NAME ) ) {

        mFontName = data[ JSON_KEY_FONT_NAME ];
    }
...
void GeneratorConfig::emitVerbose() const {
    ...
    for ( const auto& inputFont : mInputFonts ) {
